function [status, bias_out, rmse_out, rmse_em_out] = CSest(f_nameout, Dist_setup, rept, obs, Level)
...
for i=1:rep
    ...
    if Level
        ...
        [BetahWL(i,:) BetaSEWL(i,:) J_L(i,:) J_PvalL(i,:)] = genBetaWD_cr(n,t,Xdim,D2,WD_order,Xnames,Zetadim,year);
...
Beta_mL = mean(BetahWL,1); %Note! You change this line to switch between means and medians.
%EM
Beta_varL = var(BetahWL,1);
%
Beta_mSEL = mean(BetaSEWL,1);
Beta_mW = mean(BetahWW,1);
%EM
Beta_varW = var(BetahWW,1);
%
...
Beta_mOLSL = Beta_mL(1:t*Xdim);
%EM
Beta_varOLSL=Beta_varL(1:t*Xdim);
%
Beta_mSEOLSL = Beta_mSEL(1:t*Xdim);
Beta_mOLSW = Beta_mW(1:t*Xdim);
%EM
Beta_varOLSW=Beta_varW(1:t*Xdim);
%
Beta_mSEOLSW = Beta_mSEW(1:t*Xdim);

    %To order GMM EW estimates better - (t1: GMM3, GMM4, ... t2: GMM3, GMM4, ...); each GMMi in natural order
Beta_mGMML = zeros(WD_order, Xdim,t);
%EM
Beta_varGMML=zeros(WD_order, Xdim,t);
%
Beta_mSEGMML = zeros(WD_order, Xdim,t);
Beta_mGMMW = zeros(WD_order, Xdim,t);
Beta_mSEGMMW = zeros(WD_order, Xdim,t);
Beta_aux = Beta_mL( t*Xdim+1:t*Xdim*(1+WD_order) );
%EM
Beta_varaux=Beta_varL( t*Xdim+1:t*Xdim*(1+WD_order) );
%
Beta_SEaux = Beta_mSEL( t*Xdim+1:t*Xdim*(1+WD_order) );
Beta_aux1 = Beta_mW( t*Xdim+1:t*Xdim*(1+WD_order) );
Beta_SEaux1 = Beta_mSEW( t*Xdim+1:t*Xdim*(1+WD_order) );
if WD_order > 1
    tdim = WD_order + WD_order + Zetadim*WD_order;
    for T=1:t
        Baux = Beta_aux((T-1)*tdim+1:T*tdim);
        %EM
        Baux_var=Beta_varaux((T-1)*tdim+1:T*tdim);
        %
        BauxSE = Beta_SEaux((T-1)*tdim+1:T*tdim);
        Baux1 = Beta_aux1((T-1)*tdim+1:T*tdim);
        Baux1SE = Beta_SEaux1((T-1)*tdim+1:T*tdim);
        for i=1:WD_order
            Beta_mGMML(i,1,T) = Baux(i);
            %EM
            Beta_varGMML(i,1,T)=Baux_var(i);
            %
            Beta_mSEGMML(i,1,T) = BauxSE(i);
            Beta_mGMML(i,2,T) = Baux(WD_order + i);
            %EM
            Beta_varGMML(i,2,T)=Baux_var(WD_order + i);
            %
            Beta_mSEGMML(i,2,T) = BauxSE(WD_order + i);
            Beta_mGMML(i,3:end,T) = Baux((2+(i-1)) * WD_order + 1:(2+(i-1)) * WD_order + Zetadim );
            %EM
            Beta_varGMML(i,3:end,T)=Baux_var((2+(i-1)) * WD_order + 1:(2+(i-1)) * WD_order + Zetadim );;
            %
...
status = fclose(fid);
%Out Bias and RMSE figures for GMM
bias_out=Bias_GMML;
rmse_out=RMSE_GMML;
rmse_em_out=RMSE_GMML_EM;
