...
while year<=nyr;
    ...
    while estim<=nestim;
        ...
        if estim == 1;
             ...
             c(1,1)=ey2x_d/eyx2_d;
             if cstart~=0 & estim>1; c(1,1)=cstart; end;

             goodstartval = 1;

             ckeep   = zeros(goodstartval+1,nc);
             objkeep = zeros(goodstartval+1,1);
             for mmm=1:(goodstartval+1);
             ...
             end;    %----------------End of starting value loop-----------------
             if goodstartval==1;
                cidx = 1;
                if objkeep(2,1) <objkeep(1,1);
                   cidx = 2;
                end;
                c = ckeep(cidx,:)';
             end;
        ...
        objsave = f'*w*f*n;                                   %This line added by TW.

        funame = 'objfns.out';
        global fid1;
        fid1 = fopen(funame, 'a');
        fustr = 'GMM objective function %8.3f %8.3f %8.3f \n';
        fprintf(fid1, fustr, [c(1,1) estim objsave]);

        fid1 = fclose(fid1);
        %dlmwrite('myfile.txt', ojbsave, '-append')                                %This line too.
