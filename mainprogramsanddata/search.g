...
@ Step 2: ------Name of the output file.-------------------------------------@
    ...
    if fe == 0;
    output file = ewsearch.out reset;
    else;
    output file = ewFEsearch.out reset;
    endif;

    elseif assets == 1;

    if fe == 0;
    output file = ewAsearch.out reset;
    else;
    output file = ewFEAsearch.out reset;
    endif;
...
cls;
   ...
   alldta = selif(alldta,alldta[.,indcv(yrname,vnames)].>=fyear .and alldta[.,indcv(yrname,vnames)].<=(fyear+nyr-1));
   ...
   do until year > nyr;
    ...
    muy = (invpd(moment(iz,0)))* (iz'y);        @  mux';;muy';  @
    y_d = y - iz*muy;
    x_d = x - iz*mux;
...
/* The OLS estimator: */
    ...
    /* White
    u2 = uhat^2;
    desi = icept~x~za~(x.*za)~(x^2)~(za^2);
    ddi = invpd(moment(desi,0));
    auxb = ddi*desi'u2;
    uhati = u2-desi*auxb;
    R2 = 1 - moment(uhati,0)/(moment((u2 - meanc(u2)),0));
    BP = R2*n;
    "White test";; cdfchic(bp,5);
             */
...
elseif estim==5;
 ...
 neq = 29;
   ...
   objsave = zeros(mcount+1,5);
 /*  if bsave[1,year] >0;
     startgini = bsave[1,year];
     endgini   = rbsave[1,year];     @ "endpoints";; startgini;;endgini; @
   else;
     endgini   = bsave[1,year];
     startgini = rbsave[1,year];
   endif;
     if bsave[1,year]<0;
     ...
     startup = startgini - ginistretch;
     endup   = endgini   + ginistretch;     */
     startup =  -0.1;
     endup    =  0.5;

     increm  = abs(startup-endup)/(mcount-1);

   icount = 1; do while icount<=(mcount+2);
   ...
   if icount<(mcount+2);
      ...
      if estim > 1;
                         screen off;print;  "---------------------------";;estim+2;;year;;"-------------------------------";@ objsave; @screen on;
        @  objsave = selif(objsave,objsave[.,3].>(startup) .and objsave[.,3].<endup);    @
          if cols(objsave)>1;  objsave;
             objpick = abs(objsave[.,4]).<1@.1 .and abs(objsave[.,5]).<1.1@;
             @objsave = selif(objsave,objpick); @
             cbegin = selif(objsave[.,2],abs(objsave[.,1]-minc(objsave[.,1])).<0.00001);
             if rows(cbegin)>1;  cbegin = cbegin[ceil(rows(cbegin)/2)];  endif;
             c[1,1] = cbegin;
          else;
             c[1,1] = bsave[1,year];
          endif;
...
/*  Now save both things. */
    ...
    if icount<(mcount+2);
    if estim > 1; objsave[icount,1] = obj*n; endif;
    objsave[icount,2] = startup + increm*(icount - 1);
    objsave[icount,3] = c[1,1];
    objsave[icount,4] = rho;
    objsave[icount,5] = tau;
    endif;


    icount = icount + 1;
    endo;   @ End of the "monte carlo" loop@
       @
    cbegin = selif(objsave[.,2],objsave[.,1].==minc(objsave[.,1]));
    cbegin = cbegin[1,1];
    cend   = selif(objsave[.,3],objsave[.,1].==minc(objsave[.,1]));
    cend   = cend[1,1];
    objbegin = selif(objsave[.,1],objsave[.,1].==minc(objsave[.,1]));
    objbegin = objbegin[1,1];

    format /rd 25,16;
    cbegin;;objbegin;;cend;
        @
...
do while estim <= nestim;
  ...
  if panel == 1;
     ...
     w = invpd(moment(allinfl,0)./(divideby^2).*(sharefrac^1).*((sharefrac')^1));    if param==4; diag(w)~((saveit[1,idx]./saveit[2,idx])');  endif;
     @
     allinfl = miss(allinfl,0);

     for www(1,cols(allinfl),1);
        impu = meanc(packr(allinfl[.,www]));
        allinfl[.,www] = missrv(allinfl[.,www],impu);
     endfor;
     w = invpd(moment(allinfl,0)/(rows(allinfl)^2));
     @
  endif;
    ...
    ccsave[3,estim] = cdfchic(cmdtest,nyr-1); ccsave[4,estim] = stderror;
    elseif param == 2;  cpsave[1,estim] = theta; cpsave[2,estim] = cmdtest;
    cpsave[3,estim] = cdfchic(cmdtest,nyr-1); cpsave[4,estim] = stderror;
    elseif param == 3;  ctsave[1,estim] = theta; ctsave[2,estim] = cmdtest;
    ctsave[3,estim] = cdfchic(cmdtest,nyr-1); ctsave[4,estim] = stderror;
    elseif param ==nz+4;casave[1,estim] = theta; casave[2,estim] = cmdtest;
    casave[3,estim] = cdfchic(cmdtest,nyr-1); casave[4,estim] = stderror;
    endif;
...
do while param <= nz+3;
   ...
   if panel ==   1;
     ...
     w = invpd(moment(allinfl,0)./(divideby^2).*(sharefrac^1).*((sharefrac')^1));
   endif;
...
"---------------------------------------------------------------------------";
  ...
  print; "Fama-MacBeth"; print;
   ...
   "& ";; meanc(isave[1,.]');; for jj(1, nestim, 1);   " & " meanc(asave[1,pick+jj-1]');;     endfor; " \\" "\\";
   "&(";;  stdc(isave[1,.]')/sqrt(nyr);; for jj(1, nestim, 1);   ")&("  stdc(asave[1,pick+jj-1]')/sqrt(nyr);;     endfor; ") \\" "\\";


   print;
   "FM Coefficients on the mismeasured regressors:";
   print;


   "& ";; meanc(bsave[1,.]');; for jj(1, nestim, 1);   " & " meanc(csave[1,pick+jj-1]');;     endfor; " \\" "\\";
   "&(";;  stdc(bsave[1,.]')/sqrt(nyr);; for jj(1, nestim, 1);   ")&("  stdc(csave[1,pick+jj-1]')/sqrt(nyr);;     endfor; ") \\" "\\";


   print;
   "FM Coefficients on the perfectly measured regressor:";
   print;

   for ee(1, nz, 1);
      "& ";; meanc(zsave[(ee-1)*2+1,.]');; for jj(1, nestim, 1);   " & " meanc(dsave[(ee-1)*2+1,pick+jj-1]');;     endfor; " \\" "\\";
      "&(";;  stdc(zsave[(ee-1)*2+1,.]');; for jj(1, nestim, 1);   ")&("  stdc(dsave[(ee-1)*2+1,pick+jj-1]')/sqrt(nyr);;     endfor; ") \\" "\\";
   endfor;
   ...
   "& ";; meanc(rsave[1,.]');; for jj(1, nestim, 1);   " & " meanc(psave[1,pick+jj-1]');;     endfor; " \\" "\\";
   "&(";;  stdc(rsave[1,.]');; for jj(1, nestim, 1);   ")&("  stdc(psave[1,pick+jj-1]')/sqrt(nyr);;     endfor; ") \\" "\\";

   print;
   "FM tau-squared:";
   print;

   for jj(1, nestim, 1);   "& ";; meanc(tsave[1,pick+jj-1]');; " ";;    endfor; " \\" "\\";
   for jj(1, nestim, 1);   "&(";;  stdc(tsave[1,pick+jj-1]')/sqrt(nyr);; ")";;    endfor; " \\" "\\";
   ...
   print;print;
   "Starting values for GMM3 to GMM5";
    seqa(fyear,1,nyr)~startsave;
