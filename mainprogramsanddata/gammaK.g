...
format /rd 8,3;
if dofe == 0;
output file = gam_K.out reset;
else;
output file = gam_K_within.out reset;
endif;
outwidth 256;
rezat = 0;
do while rezat <=0;
...
@------------------Alternate FM Monte Carlo Counters----------------@
...
_sumg = zeros(1,nestim);
_sumer2g = zeros(1,nestim);
_sumerrg = zeros(1,nestim);
_sumseg = zeros(1,nestim);
_pcountg = zeros(1,nestim);

_sums = zeros(1,nestim);
_sumer2s = zeros(1,nestim);
_sumerrs = zeros(1,nestim);
_sumses = zeros(1,nestim);
_pcounts = zeros(1,nestim);

_sumd =      zeros(nz,nestim);
_sumer2d =  zeros(nz,nestim);
_sumerrd =   zeros(nz,nestim);
_sumsed  =   zeros(nz,nestim);
_pcountd =   zeros(nz,nestim);

_sump =  zeros(1,nestim);
_sumer2p = zeros(1,nestim);
_sumerrp = zeros(1,nestim);
_sumsep = zeros(1,nestim);
_pcountp = zeros(1,nestim);

_sumt = zeros(1,nestim);
_sumer2t = zeros(1,nestim);
_sumerrt = zeros(1,nestim);
_sumset = zeros(1,nestim);
_pcountt = zeros(1,nestim);
...
teebeta  = zeros(nestim,1);
teealpha = zeros(nestim,1);
teerho   = zeros(nestim,1);
teetau   = zeros(nestim,1);
teeiv    = zeros(2,1);
teeab    = zeros(2,1);
...
bttdb:
 ...
 @ original@
   ...
   if keepnyr>1 or nyr > 1;
    ...
    allc = reshape(allc,nncount,keepnyr); allc = allc - meanc(allc'); allc = vec(allc');
...
if checkmoms == 0;
    ...
    if keepnyr>2;
         ...
         des = ones(rows(yt1),1)~(xt0-xt1)~(zt0-zt1);
            ...
            wmx = clust((ehat.*ivs),nncount,rows(yt0)/nncount);
...
/*  Do the test that the true coefficients are zero. */

teealpha = teealpha  + (abs(((dcmd[1,.]-delta)')./(dcmd[2,.]')) .> twocrit);
teebeta  = teebeta   + (abs(((ccmd[1,.]-beta )')./(ccmd[2,.]')) .> twocrit);
teerho   = teerho    + (abs(((pcmd[1,.]-rho2 )')./(pcmd[2,.]')) .> twocrit);
teetau   = teetau    + (abs(((tcmd[1,.]-tau2 )')./(tcmd[2,.]')) .> twocrit);

teeiv[1,1]  = teeiv[1,1] + (abs((biv[2,1]-beta  )./sbiv[2,1]) .> twocrit);
teeiv[2,1]  = teeiv[2,1] + (abs((biv[3,1]-delta )./sbiv[3,1]) .> twocrit);

teeab[1,1]  = teeab[1,1] + (abs((bnew[2,1]-beta  )./sbnew[2,1]) .> twocrit);
teeab[2,1]  = teeab[2,1] + (abs((bnew[3,1]-delta )./sbnew[3,1]) .> twocrit);
...
Fepsilon         =  meanc(epsilon[idx+yy-1,1]);
Vepsilon         = (meanc(epsilon[idx+yy-1,1]^2) - Fepsilon^2)@*ncount/(ncount - 1)@;
SDepsilon        =  sqrt(Vepsilon);
SDepsilon        =  sqrt(Vepsilon);
thirdepsilon     =  meanc((epsilon[idx+yy-1,1] - Fepsilon)^3);
skewepsilon      =  thirdepsilon/(SDepsilon^3);
fourthepsilon    =  meanc((epsilon[idx+yy-1,1] - Fepsilon)^4);
kurtepsilon      =  (fourthepsilon/(SDepsilon^4));
fifthepsilon     =  meanc((epsilon[idx+yy-1,1] - Fepsilon)^5);
std5epsilon      =  (fifthepsilon)/(SDepsilon^5);
sixthepsilon     =  meanc((epsilon[idx+yy-1,1] - Fepsilon)^6);
std6epsilon      =  (sixthepsilon)/(SDepsilon^6);
sevenepsilon     = meanc((epsilon[idx+yy-1,1] - Fepsilon)^7);
std7epsilon      = (sevenepsilon)/(SDepsilon^7);
...
you = u;
@whytwoex = allydot[idx+yy-1,1]^2).*(allxdot[idx+yy-1   @
...
avAepsilon    = avAepsilon + Fepsilon;
avVepsilon    = avVepsilon + Vepsilon;
avSkewepsilon = avSkewepsilon + skewepsilon;
avKurtepsilon = avKurtepsilon + kurtepsilon;
avstd5epsilon = avstd5epsilon + std5epsilon;
avstd6epsilon = avstd6epsilon + std6epsilon;
avstd7epsilon = avstd7epsilon + std7epsilon;
...
if ceil(nrun/10) == nrun/10 and nrun /=0;
@locate 3,1;   @
format /rd 6,3;
...
print " Probability that the hypothesis (beta - beta(true)) /= 0 is rejected.     ";
print " Columns are estimators. Rows are significance levels of   ";
print " 0.05 ";

for qq(1, nestim, 1); "&";; teebeta[qq,1]/nrun;; endfor; "\\" "\\";
...
for qq(1, nestim, 1); "&";; teealpha[qq,1]/nrun;; endfor; "\\" "\\";




print " Probability that the hypothesis (rho2 - rho2(true)) /= 0 is rejected.     ";
print " Columns are estimators. Rows are significance levels of   ";
print " 0.05 ";

for qq(1, nestim, 1); "&";; teerho[qq,1]/nrun;; endfor; "\\" "\\";




print " Probability that the hypothesis (tau2 - tau2(true)) /= 0 is rejected.     ";
print " Columns are estimators. Rows are significance levels of   ";
print " 0.05 ";

for qq(1, nestim, 1); "&";; teetau[qq,1]/nrun;; endfor; "\\" "\\";


print " Probability that the hypothesis (beta - beta(true)) /= 0 is rejected.     ";
print " IV and AB";

teeiv[1,1]/nrun;; teeab[1,1]/nrun;; "\\" "\\";


print " Probability that the hypothesis (alpha_1 - alpha_1(true)) /= 0 is rejected.     ";
print " IV and AB";

teeiv[2,1]/nrun;; teeab[2,1]/nrun;; "\\" "\\";
...
print; "EPSILON"; print;
"&" avAepsilon/(keepnyr*nrun);; "&" avVepsilon/(keepnyr*nrun);; "&" avskewepsilon/(keepnyr*nrun);; "&" avkurtepsilon/(keepnyr*nrun);;
"&" avstd5epsilon/(keepnyr*nrun);; "&" avstd6epsilon/(keepnyr*nrun);; "&" avstd7epsilon/(keepnyr*nrun);; "\\" "\\";
