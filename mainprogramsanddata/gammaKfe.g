...
format /rd 8,3;
if dofe == 0;
output file = gam_Kfe.out reset;
else;
output file = gam_Kfe_within.out reset;
endif;
outwidth 256;
...
bttdb:
   ...
   if keepnyr>1 or nyr > 1;
       creg = reshape(creg,ncount,nyr); creg = creg[.,nyr-keepnyr+1:nyr]; fixchi = 1/@sqrt@(keepnyr)*sumc(creg'); creg = vec(creg');
       zreg = reshape(zreg,ncount,nyr); zreg = zreg[.,nyr-keepnyr+1:nyr]; fixz   = @sqrt(keepnyr)*@meanc(zreg');  zreg = vec(zreg');
       ...
       fixchi = fixchi.*.ones(keepnyr,1)*sqrt(1-qrho^2);
       fixz   = fixz.*.ones(keepnyr,1)*sqrt(1-zrho^2);
       fixeff = (fixchi + fixz)/(0.65*rt2);




    allc     = creg*Sdchi + EofX;
    u        = (uk+corrfe*fixeff)/rt2*Sdu;
    epsilon  = ek*Sde;
...
fourthchi =  meanc((allc[idx+yy-1,1] - Eki)^4);
kurtchi   =  (fourthchi/(SDki^4));
fifthchi  =  meanc((allc[idx+yy-1,1] - Eki)^5);
std5chi   =  (fifthchi)/(SDki^5);
sixthchi  =  meanc((allc[idx+yy-1,1] - Eki)^6);
...
print;
"corrfe = ";; corrfe;; "J-test";; EWjcount/nrun;
print;
...
"beta   corrfe = ";; corrfe;; "&"  sumb/nrun/beta-1;;               "&"  sumbiv/nrun/beta-1;;               "&"  sumbab/nrun/beta-1;;                  for qq (1, nestim, 1); "&" sumg[1,qq]/nrun/beta-1;;          endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&"  median(bkeep[1:nrun,.])/beta-1;; "&"  median(bivkeep[1:nrun,.])/beta-1;; "&"  median(babkeep[1:nrun,.])/beta-1;;    for qq (1, nestim, 1); "&" median(gkeep[1:nrun,qq])/beta-1;; endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&"  sumerrb/nrun;;                   "&"  sumerrbiv/nrun;;                   "&"  sumerrbab/nrun;;                      for qq (1, nestim, 1); "&" sumerrg[1,qq]/nrun;;              endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&"  pcountb[1,1]/nrun;;              "&"  pcountbiv[1,1]/nrun;;              "&"  pcountbab[1,1]/nrun;;                 for qq (1, nestim, 1); "&" pcountg[1,qq]/nrun;;              endfor; "\\" "\\";; " [11pt] ";


for ee(1, nz, 1);
"alpha  corrfe = ";; corrfe;; "&" sumz[ee,1]/nrun/delta[ee,1]-1;;          "&" sumziv/nrun/delta[ee,1]-1;;                "&" sumzab/nrun/delta[ee,1]-1;;               for qq (1, nestim, 1); "&" sumd[ee,qq]/nrun/delta[ee,1]-1;;                       endfor; "\\" "\\";; " [2pt] ";
"median corrfe = ";; corrfe;; "&" median(zkeep[1:nrun,ee])/delta[ee,1]-1;; "&" median(zivkeep[1:nrun,.])/delta[ee,1]-1;;  "&" median(zabkeep[1:nrun,.])/delta[ee,1]-1;; for qq (1, nestim, 1); "&" median(dkeep[1:nrun,(ee-1)*nestim+qq])/delta[ee,1]-1;; endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&" sumerrz[ee,1]/nrun;;                     "&" sumerrziv/nrun;;                           "&" sumerrzab/nrun;;                          for qq (1, nestim, 1); "&" sumerrd[ee,qq]/nrun;;                                  endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&" pcountz[ee,1]/nrun;;                     "&" pcountziv[1,1]/nrun;;                      "&" pcountzab[1,1]/nrun;;                     for qq (1, nestim, 1); "&" pcountd[ee,qq]/nrun;;                                  endfor; "\\" "\\";; " [11pt] ";
endfor;

"rho2   corrfe = ";; corrfe;; "&" sumARR/nrun/rho2-1;;              "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" sump[1,qq]/nrun/rho2-1;;          endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&"  median(rkeep[1:nrun,.])/rho2-1;; "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" median(pkeep[1:nrun,qq])/rho2-1;; endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&" sumerrr/nrun;;                    "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" sumerrp[1,qq]/nrun;;              endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&" pcountr/nrun;;                    "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" pcountp[1,qq]/nrun;;              endfor; "\\" "\\";; " [11pt] ";

"tau2   corrfe = ";; corrfe;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" sumt[1,qq]/nrun/tau2-1;;          endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" median(tkeep[1:nrun,qq])/tau2-1;; endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" sumerrt[1,qq]/nrun;;              endfor; "\\" "\\";; " [2pt] ";
"       corrfe = ";; corrfe;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" pcountt[1,qq]/nrun;;              endfor; "\\" "\\";; " [11pt] ";

"------------------------------CMD----------------------------------------------";

"beta  corrfe = ";; corrfe;; "&"  sumb/nrun;;               "&"  sumbiv/nrun;;               "&"  sumbab/nrun;;                  for qq (1, nestim, 1); "&" sumg[1,qq]/nrun;;          endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&"  median(bkeep[1:nrun,.]);; "&"  median(bivkeep[1:nrun,.]);; "&"  median(babkeep[1:nrun,.]);;    for qq (1, nestim, 1); "&" median(gkeep[1:nrun,qq]);; endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&"  sumerrb/nrun;;            "&"  sumerrbiv/nrun;;            "&"  sumerrbab/nrun;;               for qq (1, nestim, 1); "&" sumerrg[1,qq]/nrun;;       endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&"  pcountb[1,1]/nrun;;       "&"  pcountbiv[1,1]/nrun;;       "&"  pcountbab[1,1]/nrun;;          for qq (1, nestim, 1); "&" pcountg[1,qq]/nrun;;       endfor; "\\" "\\";; " [11pt] ";


for ee(1, nz, 1);
"alpha corrfe = ";; corrfe;; "&" sumz[ee,1]/nrun;;          "&" sumziv/nrun;;                "&" sumzab/nrun;;               for qq (1, nestim, 1); "&" sumd[ee,qq]/nrun;;                       endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&" median(zkeep[1:nrun,ee]);; "&" median(zivkeep[1:nrun,.]);;  "&" median(zabkeep[1:nrun,.]);; for qq (1, nestim, 1); "&" median(dkeep[1:nrun,(ee-1)*nestim+qq]);; endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&" sumerrz[ee,1]/nrun;;       "&" sumerrziv/nrun;;             "&" sumerrzab/nrun;;            for qq (1, nestim, 1); "&" sumerrd[ee,qq]/nrun;;                    endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&" pcountz[ee,1]/nrun;;       "&" pcountziv[1,1]/nrun;;        "&" pcountzab[1,1]/nrun;;       for qq (1, nestim, 1); "&" pcountd[ee,qq]/nrun;;                    endfor; "\\" "\\";; " [11pt] ";
endfor;

"rho2  corrfe = ";; corrfe;; "&" sumARR/nrun;;                     "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" sump[1,qq]/nrun;;          endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&"  median(rkeep[1:nrun,.]);;        "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" median(pkeep[1:nrun,qq]);; endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&" sumerrr/nrun;;                    "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" sumerrp[1,qq]/nrun;;       endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&" pcountr/nrun;;                    "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" pcountp[1,qq]/nrun;;       endfor; "\\" "\\";; " [11pt] ";

"tau2  corrfe = ";; corrfe;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" sumt[1,qq]/nrun;;          endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" median(tkeep[1:nrun,qq]);; endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" sumerrt[1,qq]/nrun;;       endfor; "\\" "\\";; " [2pt] ";
"      corrfe = ";; corrfe;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" pcountt[1,qq]/nrun;;       endfor; "\\" "\\";; " [11pt] ";
...
"IV beta  corrfe = ";; corrfe;; "&"  sumbiv/nrun;;               "&"  sumbab/nrun;
"         corrfe = ";; corrfe;; "&"  median(bivkeep[1:nrun,.]);; "&"  median(babkeep[1:nrun,.]);
"         corrfe = ";; corrfe;; "&"  sumerrbiv/nrun;;            "&"  sumerrbab/nrun;
"         corrfe = ";; corrfe;; "&"  pcountbiv[1,1]/nrun;;       "&"  pcountbab[1,1]/nrun;

for ee(1, nz, 1);
"IV alpha corrfe = ";; corrfe;; "&" sumziv/nrun;;                "&" sumzab/nrun;
"         corrfe = ";; corrfe;; "&" median(zivkeep[1:nrun,.]);;  "&" median(zabkeep[1:nrun,.]);
"         corrfe = ";; corrfe;; "&" sumerrziv/nrun;;             "&" sumerrzab/nrun;
"         corrfe = ";; corrfe;; "&" pcountziv[1,1]/nrun;;        "&" pcountzab[1,1]/nrun;
endfor;
