...
format /rd 8,3;
if dofe == 0;
output file = gam_Kcorre.out reset;
else;
output file = gam_Kcorre_within.out reset;
endif;
outwidth 256;
...
samplesize = 2;
do while samplesize <=2;


if samplesize == 1;       ncount = 500;
elseif samplesize == 2;   ncount = 1500;
elseif samplesize == 3;   ncount = 2000;
endif;
...
bttdb:
    ...
    creg    = zeros(ncount*nyr,1);
    zreg    = zeros(ncount*nyr,1);
    ek      = zeros(ncount*nyr,1);
    uk      = zeros(ncount*nyr,1);
    ...
    { einnov, s4 } = rndLCgam(ncount*nyr, 1, agamE, s4);      einnov = bgamE*einnov;              einnov =  (einnov   - agamE*bgamE)/sqrt(agamE*bgamE^2);
    { uinnov, s5 } = rndLCgam(ncount*nyr, 1, agamU, s5);      uinnov = bgamU*uinnov;              uinnov =  (uinnov   - agamU*bgamU)/sqrt(agamU*bgamU^2);
    { rinnov, s1 } = rndLCgam(ncount*nyr, 1, agamR, s1);      rinnov = bgamR*rinnov;              rinnov =  (rinnov -   agamR*bgamR)/sqrt(agamR*bgamR^2);
    { zinnov, s3 } = rndLCgam(ncount*nyr, 1, agamZ, s3);      zinnov = bgamZ*zinnov;              zinnov = -(zinnov   - agamZ*bgamZ)/sqrt(agamZ*bgamZ^2);

    idx = seqa(1,nyr,ncount);                                @        This just indexes the different firms in the matrix. They are stacked on top of each other.    @
    { ek[idx,1],   s12   } = rndLCgam(ncount, 1, agamE,s12);    ek[idx,1]   = bgamE*ek[idx,1];      ek[idx,1]   =    (ek[idx,1]   - agamE*bgamE)/sqrt(agamE*bgamE^2);
    { uk[idx,1],   s11   } = rndLCgam(ncount, 1, agamU,s11);    uk[idx,1]   = bgamU*uk[idx,1];      uk[idx,1]   =    (uk[idx,1]   - agamU*bgamU)/sqrt(agamU*bgamU^2);
    { creg[idx,1], s10 }   = rndLCgam(ncount, 1, agamR,s10);    creg[idx,1] = bgamR*creg[idx,1];    creg[idx,1] =  (creg[idx,1]   - agamR*bgamR)/sqrt(agamR*bgamR^2);
    { zreg[idx,1], s2  }   = rndLCgam(ncount, 1, agamZ,s2 );    zreg[idx,1] = bgamZ*zreg[idx,1];    zreg[idx,1] = -(zreg[idx,1]   - agamZ*bgamZ)/sqrt(agamZ*bgamZ^2);
    yr = zeros(ncount*nyr,1);
    yr[idx,1] = ones(ncount,1);
   ...
   if keepnyr>1 or nyr > 1;
       creg = reshape(creg,ncount,nyr); creg = creg[.,nyr-keepnyr+1:nyr];  creg = vec(creg');
       zreg = reshape(zreg,ncount,nyr); zreg = zreg[.,nyr-keepnyr+1:nyr];  zreg = vec(zreg');
       uk = reshape(uk,ncount,nyr); uk = uk[.,nyr-keepnyr+1:nyr]; uk = vec(uk');
       ek = reshape(ek,ncount,nyr); ek = ek[.,nyr-keepnyr+1:nyr]; ek = vec(ek');
       yr = reshape(yr,ncount,nyr); yr = yr[.,nyr-keepnyr+1:nyr]; yr = vec(yr');  yr = yr - nyr + keepnyr;
  endif;
    ...
    alliz = ones(rows(allz),1)~allz;
    mux = (invpd(moment(alliz,0)))* (alliz'allx);
    muy = (invpd(moment(alliz,0)))* (alliz'ally);


    allydot = ally - alliz*muy;
    allxdot = allx - alliz*mux;



    if dofe == 1;

    allx = reshape(allx,ncount,keepnyr); allx = allx - meanc(allx'); allx = vec(allx');
    ally = reshape(ally,ncount,keepnyr); ally = ally - meanc(ally'); ally = vec(ally');
    allz = reshape(allz,ncount,keepnyr); allz = allz - meanc(allz'); allz = vec(allz');

    endif;

    idx = seqa(1,keepnyr,ncount);                                           @This just indexes the different firms in the matrix. They are stacked on top of each other. @
    year = 1;
...
/* The OLS estimator: */
    ...
    olsinflnc = invpd(moment(des,0)/ncount)*((des.*uhat)');
    ...
    for qq(1, nz, 1); izsave[((qq-1)*ncount+1):(qq*ncount),year] = olsinflnc[2+qq,.]'; endfor;
...
Exza = meanc(xza);
nreg=nz+1;
sigxz = moment((xza)-Exza',0)/ncount;
...
phixz = zeros(ncount,nreg+nreg*(nreg-1)/2);
...
elseif estim==5;
 ...
 neq = 29;
    ...
    c[1,1] = Ey2x_d/Eyx2_d;  if betastart == 1;  if estim > 1; c[1,1] = beta;endif; endif;


if estim>1;

    @
  if abs(c[1,1]) < abs(bsave[1,year]) or abs(c[1,1]) > abs(rb);
     c[1,1] = bsave[1,year];
  endif;
  @

 if abs(c[1,1])<abs(allb[2,1]);
   c[1,1] = allb[2,1];
 elseif abs(c[1,1])>abs(rb);
   c[1,1] = allb[2,1];
 endif;
...
if estim>1;
  ...
  w = invit(moment(ff,0)./ncount);
  if w == -999999; goto bttdb; endif;
...
@ Compute t-ratios, etc.                                            @
  ...
  ff = optw(c,mom,Emom,bleh);
  ffee = optw(c,mom,Emom,1);
  f = deff(c,Emom,0);

  w = invit(moment(ff,0)./ncount);
  if w == -999999; goto bttdb; endif;
  if estim > 1;
     hansenj[1,estim] = ncount*f'w*f-oidcrit[estim,1];
  endif;
  vv = moment(ffee,0)./ncount;
  ...
  gwvwg = g'w*vv*w*g;
  vc=invit(gwg)/ncount;
  if vc == -999999; goto bttdb; endif;
     if (not(diag(vc) > 0));
        goto bttdb;
        endif;
      if (not(diag(ncount*vc*gwvwg*vc) > 0));
        goto bttdb;
        endif;

    stderr=sqrt(diag(vc));
    badstd=sqrt(diag(ncount*vc*gwvwg*vc));
    inflnc=-ncount*vc*g'w*ff';
...
phiz = zeros(ncount,nz+nz*(nz-1)/2);
...
do while estim <= nestim;
  ...
  if param == 1;
    saveit = csave;
    w = invit(moment(icsave[.,idx],0)/ncount);
  elseif param == 2;
    saveit = psave;
    w = invit(moment(ipsave[.,idx],0)/ncount);
  elseif param == 3;
    saveit = tsave;
    w = invit(moment(itsave[.,idx],0)/ncount);
  endif;

  for qq(1, nz, 1);
  if param == qq+3;
    saveit = dsave[qq,.];
    w = invit(moment(idsave[((qq-1)*ncount+1):(qq*ncount),idx],0)/ncount);
    ...
    stderror = sqrt(invpd(g'w*g)/ncount);
    cmdtest = ncount*f'w*f;


    if param == 1;      ccmd[1,estim] = theta; ccmd[2,estim] =  stderror; ccmd[3,estim] =  cmdtest;
    elseif param == 2;  pcmd[1,estim] = theta; pcmd[2,estim] =  stderror; pcmd[3,estim] =  cmdtest;
    elseif param == 3;  tcmd[1,estim] = theta; tcmd[2,estim] =  stderror; tcmd[3,estim] =  cmdtest;
    endif;
...
do while param <= nz+3;
  ...
  if param == 1;
    saveit = bsave[1,.];
    w = invit(moment(ibsave,0)/ncount);
  elseif param == 2;
    saveit = Rsave[1,.];
    w = invit(moment(irsave,0)/ncount);
  elseif param == 3;
    saveit = asave[1,.];
    w = invit(moment(iasave,0)/ncount);
  endif;

    for qq(1, nz, 1);
    if param == qq+3;
      saveit = zsave[qq,.];
      w = invit(moment(izsave[((qq-1)*ncount+1):(qq*ncount),.],0)/ncount);
    endif;
    endfor;
...
@ Compute t-ratios, etc.                                            @
    ...
    for qq(1, nz, 1);
    if param == qq+3;
      zcmd[(qq-1)*3+1,1] = theta;
      zcmd[(qq-1)*3+2,1] = stderror;
      zcmd[(qq-1)*3+3,1] = cmdtest;
    endif;
    endfor;
...
if checkmoms == 0;
    ...
    if keepnyr>2;
         ...
         des = ones(rows(yt1),1)~(xt0-xt1)~(zt0-zt1);
            ...
            inezz = invpd(zzzz);
            desivs = des'ivs;
            biv = invpd(desivs*inezz*desivs')*desivs*inezz*(ivs'(yt0-yt1));
            ehat = (yt0-yt1) - des*biv;

            wmx = clust((ehat.*ivs),ncount,rows(yt0)/ncount);
         ...
         if rezat == 1;
            if biv[2,1]<minc(allb[2,1]|rb); biv[2,1]=minc(allb[2,1]|rb); endif;
            if biv[2,1]>maxc(allb[2,1]|rb); biv[2,1]=maxc(allb[2,1]|rb); endif;
            if biv[3,1]<minc(allb[3,1]|rd); biv[3,1]=minc(allb[3,1]|rd); endif;
            if biv[3,1]>maxc(allb[3,1]|rd); biv[3,1]=maxc(allb[3,1]|rd); endif;
         endif;

       @AB@

       yrshape = reshape(yr,ncount,keepnyr);
       yshape = reshape(ally,ncount,keepnyr);
       xshape = reshape(allx,ncount,keepnyr);
       zshape = reshape(allz,ncount,keepnyr);  @ yr~ally;
                                                 vec(yrshape)~vec(yshape);
                                                @
       touty = vec(yshape);
       @touty[ncount*2+1:rows(touty),1]~touty[ncount+1:rows(touty)-ncount,1];@
       touty = touty[ncount*2+1:rows(touty),1] - touty[ncount+1:rows(touty)-ncount,1];
      @ touty;end;   @

       toutx = vec(xshape);
       toutz = vec(zshape);
       toutx = (toutx[ncount*2+1:rows(toutx),1] - toutx[ncount+1:rows(toutx)-ncount,1])~(toutz[ncount*2+1:rows(toutz),1] - toutz[ncount+1:rows(toutz)-ncount,1]);
       toutx = ones(rows(toutx),1)~toutx;
       nzab = 4;
       toutz = zeros(ncount*(keepnyr-2),(keepnyr-2)*(nzab));

       xz = vec(xshape)~vec(zshape)~vec(yshape);

       for ii(1,keepnyr-2,1);
        toutz[(ii-1)*ncount+1:ii*ncount,(ii-1)*(nzab)+1:ii*(nzab)]  = ones(ncount,1)~xz[(ii-1)*ncount+1:ii*ncount,1:2]~(xz[(ii+1)*ncount+1:(ii+2)*ncount,2] - xz[(ii-0)*ncount+1:(ii+1)*ncount,2] );
       endfor;

       inezz = invpd(moment(toutz,0));
       bhat = invpd(toutx'toutz*inezz*toutz'toutx)*toutx'toutz*inezz*toutz'touty;
       ...
       bnew = invpd(toutx'toutz*iomega*toutz'toutx)*toutx'toutz*iomega*toutz'touty;
       ...
       inflnc = ncount*invpd(toutx'toutz*iomega*toutz'toutx)*toutx'toutz*iomega*toutz';
       inflnc = inflnc.*vhat';

       /* This part does the m2 test */


       x_2 = (xz[ncount*1+1:rows(xz)-ncount*2,1] - xz[ncount*0+1:rows(xz)-3*ncount,1])
            ~(xz[ncount*1+1:rows(xz)-ncount*2,2] - xz[ncount*0+1:rows(xz)-3*ncount,2]);
       y_2 = (xz[ncount*1+1:rows(xz)-ncount*2,3] - xz[ncount*0+1:rows(xz)-3*ncount,3]);
       x_0 = (xz[ncount*3+1:rows(xz)-ncount*0,1] - xz[ncount*2+1:rows(xz)-1*ncount,1])
            ~(xz[ncount*3+1:rows(xz)-ncount*0,2] - xz[ncount*2+1:rows(xz)-1*ncount,2]);
       y_0 = (xz[ncount*3+1:rows(xz)-ncount*0,3] - xz[ncount*2+1:rows(xz)-1*ncount,3]);

       x_2 = ones(rows(x_2),1)~x_2;
       x_0 = ones(rows(x_0),1)~x_0;
       ...
       /*  This does the AB serial correlation test           */

       nx = cols(x_0);

       enn = (keepnyr-3)*ncount;
       ...
       sigyy = y_0'y_2/ncount;
       sigxy = x_0'y_2/ncount;
       sigyx = y_0'x_2/ncount;
       ...
       @ cls; rows(newinflnc);cols(newinflnc);rows(inflnc);cols(inflnc);end;@


       bigphi = (inflnc[.,ncount+1:cols(inflnc)])|(phiyy')|(phixy')|(phiyx')|(phiz');

       gee = zeros(1,rows(bigphi));


       nw = nx;

       dvecsigzdbeta = sigz'bnew + sigz*bnew;

       for mmm(1,nw,1);

       gee[1,mmm] =  -sigxy[mmm,1] - sigyx[1,mmm] + dvecsigzdbeta[mmm,1];

       endfor;


       gee[1,1*rows(bnew)+1] = 1;
       gee[1,1*rows(bnew)+2:2*rows(bnew)+1] = -bnew';
       gee[1,2*rows(bnew)+2:3*rows(bnew)+1] = -bnew';
       gee[1,3*rows(bnew)+2:cols(gee)]   = vec(bnew*bnew')';


       avar = moment(bigphi',0)./(ncount^2);
       ss   = sqrt(diag(gee*avar*gee'));

       tstat = s/ss;
      @ tstat; @


       ABm2count  = ABm2count  + (cdfnc(abs(tstat)).<0.025);

      if rezat == 1;
          if bnew[2,1]<minc(allb[2,1]|rb); bnew[2,1]=minc(allb[2,1]|rb); endif;
          if bnew[2,1]>maxc(allb[2,1]|rb); bnew[2,1]=maxc(allb[2,1]|rb); endif;
          if bnew[3,1]<minc(allb[3,1]|rd); bnew[3,1]=minc(allb[3,1]|rd); endif;
          if bnew[3,1]>maxc(allb[3,1]|rd); bnew[3,1]=maxc(allb[3,1]|rd); endif;
      endif;
...
/* Update the counters. */
    ...
    sumbiv = sumbiv + biv[2,1];
    sumerr2biv = sumerr2biv + (biv[2,1] - beta)^2;
    sumerrbiv = sumerrbiv + abs(biv[2,1] - beta);
    sumsebiv = sumsebiv + sbiv[2,1];
    bivkeep[icount,.] = biv[2,1];

    sumziv     = sumziv     + biv[3,1];
    sumseziv   = sumseziv   + sbiv[3,1];
    sumerr2ziv = sumerr2ziv + (biv[3,1] - acf)^2;
    sumerrziv  = sumerrziv  + abs(biv[3,1] - acf);
    zivkeep[icount,.] = biv[3,1];

    testbiv = abs(biv[2,1] - beta);
    testziv = abs(biv[3,1] - acf);

    pcountbiv[1,1] =  pcountbiv[1,1] + (testbiv <= (0.2*beta));
    pcountziv[1,1] =  pcountziv[1,1] + (testziv <= (0.2*acf));

    sumbab = sumbab + bnew[2,1];
    sumerr2bab = sumerr2bab + (bnew[2,1] - beta)^2;
    sumerrbab = sumerrbab + abs(bnew[2,1] - beta);
    sumsebab = sumsebab + sbnew[2,1];
    babkeep[icount,.] = bnew[2,1];

    sumzab     = sumzab     + bnew[3,1];
    sumsezab   = sumsezab   + sbnew[3,1];
    sumerr2zab = sumerr2zab + (bnew[3,1] - acf)^2;
    sumerrzab  = sumerrzab  + abs(bnew[3,1] - acf);
    zabkeep[icount,.] = bnew[3,1];

    testbab = abs(bnew[2,1] - beta);
    testzab = abs(bnew[3,1] - acf);

    pcountbab[1,1] =  pcountbab[1,1] + (testbab <= (0.2*beta));
    pcountzab[1,1] =  pcountzab[1,1] + (testzab <= (0.2*acf));



/*  Do the test that alpha_1 is zero. */

sumdelta[.,1] = sumdelta[.,1] + ((((dcmd[1,.]-delta)')./(dcmd[2,.]')) .> acritval);
sumdelta[.,2] = sumdelta[.,2] + (abs(((dcmd[1,.]-delta)')./(dcmd[2,.]')) .> twocrit);
...
*/
    ...
    idx = seqa(1,keepnyr,ncount);                                           @This just indexes the different firms in the matrix. They are stacked on top of each other. @
for yy(1, keepnyr, 1);
Fy      = meanc(ally[idx+yy-1,1]);
Vy      = (meanc(ally[idx+yy-1,1]^2) - Fy^2)*ncount/(ncount - 1);
SDy     = sqrt(Vy);
thirdy  = meanc((ally[idx+yy-1,1] - Fy)^3);
skewy   = thirdy/(SDy^3);
fourthy = meanc((ally[idx+yy-1,1] - Fy)^4);
kurty   = (fourthy/(SDy^4));
fifthy  = meanc((ally[idx+yy-1,1]- Fy)^5);
std5y   = (fifthy)/(SDy^5);
sixthy  = meanc((ally[idx+yy-1,1] - Fy)^6);
std6y   = (sixthy)/(SDy^6);
seveny  = meanc((ally[idx+yy-1,1] - Fy)^7);
std7y   = (seveny)/(SDy^7);

Fx      = meanc(allx[idx+yy-1,1]);
Vx      = (meanc(allx[idx+yy-1,1]^2) - Fx^2)*ncount/(ncount - 1);
SDx     =  sqrt(Vx);
thirdx  =  meanc((allx[idx+yy-1,1] - Fx)^3);
skewx   =  thirdx/(SDx^3);
fourthx =  meanc((allx[idx+yy-1,1] - Fx)^4);
kurtx   =  (fourthx/(SDx^4));
fifthx  =  meanc((allx[idx+yy-1,1] - Fx)^5);
std5x   =  (fifthx)/(SDx^5);
sixthx  =  meanc((allx[idx+yy-1,1] - Fx)^6);
std6x   =  (sixthx)/(SDx^6);
sevenx  = meanc((allx[idx+yy-1,1] - Fx)^7);
std7x   = (sevenx)/(SDx^7);

Echi      = meanc(allc[idx+yy-1,1]);
Vchi      = (meanc(allc[idx+yy-1,1]^2) - Echi^2)*ncount/(ncount - 1);
SDki     =  sqrt(Vchi);
thirdchi  =  meanc((allc[idx+yy-1,1] - Echi)^3);
skewchi   =  thirdchi/(SDki^3);
fourthchi =  meanc((allc[idx+yy-1,1] - Echi)^4);
kurtchi   =  (fourthchi/(SDki^4));
fifthchi  =  meanc((allc[idx+yy-1,1] - Echi)^5);
std5chi   =  (fifthchi)/(SDki^5);
sixthchi  =  meanc((allc[idx+yy-1,1] - Echi)^6);
std6chi   =  (sixthchi)/(SDki^6);
sevenchi  = meanc((allc[idx+yy-1,1] - Echi)^7);
std7chi   = (sevenchi)/(SDki^7);

Fz      = meanc(allz[idx+yy-1,1]);
Vz      = (meanc(allz[idx+yy-1,1]^2) - Fz^2)*ncount/(ncount - 1);
SDz    = sqrt(Vz);
thirdz  = meanc((allz[idx+yy-1,1] - Fz)^3);
skewz   = thirdz/(SDz^3);
fourthz = meanc((allz[idx+yy-1,1] - Fz)^4);
kurtz   = (fourthz/(SDz^4));
fifthz  = meanc((allz[idx+yy-1,1] - Fz)^5);
std5z   = (fifthz)/(SDz^5);
sixthz  = meanc((allz[idx+yy-1,1] - Fz)^6);
std6z   = (sixthz)/(SDz^6);
sevenz  = meanc((allz[idx+yy-1,1] - Fz)^7);
std7z   = (sevenz)/(SDz^7);


des = ones(rows(allz[idx+yy-1,1]),1)~allx[idx+yy-1,1]~allz[idx+yy-1,1];
bee = invpd(moment(des,0))*des'ally[idx+yy-1,1];
you = ally[idx+yy-1,1] - des*bee;

Eu      = meanc(you);
Vu      = (meanc(you^2) - Eu^2)*ncount/(ncount - 1);
SDyou    = sqrt(Vu);
thirdu  = meanc((you - Eu)^3);
skewu   = thirdu/(SDyou^3);
fourthu = meanc((you - Eu)^4);
kurtu   = (fourthu/(SDyou^4));
fifthu  = meanc((you - Eu)^5);
std5u   = (fifthu)/(SDyou^5);
sixthu  = meanc((you - Eu)^6);
std6u   = (sixthu)/(SDyou^6);
sevenu  = meanc((you - Eu)^7);
std7u   = (sevenu)/(SDyou^7);


avAy    = avAy + Fy;
avVy    = avVy + Vy;  @  icount;;vy;; @
avSkewy = avSkewy + skewy;
avKurty = avKurty + kurty;
avstd5y = avstd5y + std5y;
avstd6y = avstd6y + std6y;
avstd7y = avstd7y + std7y;

avAx    = avAx + Fx;
avVx    = avVx + Vx;           @     vx;;  @
avSkewx = avSkewx + skewx;
avKurtx = avKurtx + kurtx;
avstd5x = avstd5x + std5x;
avstd6x = avstd6x + std6x;
avstd7x = avstd7x + std7x;


avAchi    = avAchi + Echi;
avVchi    = avVchi + Vchi;
avSkewchi = avSkewchi + skewchi;
avKurtchi = avKurtchi + kurtchi;
avstd5chi = avstd5chi + std5chi;
avstd6chi = avstd6chi + std6chi;
avstd7chi = avstd7chi + std7chi;

avAz    = avAz + Fz;
avVz    = avVz + Vz;                @    vz;      @
avSkewz = avSkewz + skewz;
avKurtz = avKurtz + kurtz;
avstd5z = avstd5z + std5z;
avstd6z = avstd6z + std6z;
avstd7z = avstd7z + std7z;

avAu    = avAu + Eu;
avVu    = avVu + Vu;
avSkewu = avSkewu + skewu;
avKurtu = avKurtu + kurtu;
avstd5u = avstd5u + std5u;
avstd6u = avstd6u + std6u;
avstd7u = avstd7u + std7u;


/*  Make the summary statistics for the deviations */


Vy      = meanc(allydot[idx+yy-1,1]^2)*ncount/(ncount - 1);
SDy     = sqrt(Vy);
thirdy  = meanc((allydot[idx+yy-1,1])^3);
skewy   = thirdy/(SDy^3);
fourthy = meanc((allydot[idx+yy-1,1])^4);
kurty   = (fourthy/(SDy^4));
fifthy  = meanc((allydot[idx+yy-1,1])^5);
std5y   = (fifthy)/(SDy^5);
sixthy  = meanc((allydot[idx+yy-1,1])^6);
std6y   = (sixthy)/(SDy^6);
seveny  = meanc((allydot[idx+yy-1,1])^7);
std7y   = (seveny)/(SDy^7);

Vx      = meanc(allxdot[idx+yy-1,1]^2)*ncount/(ncount - 1);
SDx     =  sqrt(Vx);
thirdx  =  meanc((allxdot[idx+yy-1,1])^3);
skewx   =  thirdx/(SDx^3);
fourthx =  meanc((allxdot[idx+yy-1,1])^4);
kurtx   =  (fourthx/(SDx^4));
fifthx  =  meanc((allxdot[idx+yy-1,1])^5);
std5x   =  (fifthx)/(SDx^5);
sixthx  =  meanc((allxdot[idx+yy-1,1])^6);
std6x   =  (sixthx)/(SDx^6);
sevenx  =  meanc((allxdot[idx+yy-1,1])^7);
std7x   =  (sevenx)/(SDx^7);

meVy    = meVy + Vy;
meSkewy = meSkewy + skewy;
meKurty = meKurty + kurty;
mestd5y = mestd5y + std5y;
mestd6y = mestd6y + std6y;
mestd7y = mestd7y + std7y;

meVx    = meVx + Vx;
meSkewx = meSkewx + skewx;
meKurtx = meKurtx + kurtx;
mestd5x = mestd5x + std5x;
mestd6x = mestd6x + std6x;
mestd7x = mestd7x + std7x;
...
if ceil(nrun/10) == nrun/10;
@locate 3,1;   @
format /rd 6,3;
"Baseline Trial Number " nrun;

"CMD Estimator Intermediate Results";
"Unconstrained Mean Absolute Deviations";
print;
"Beta";
"     OLS:   ";; (sumerrb/nrun);
print;
...
for qq(1, nestim, 1); "&";; sumdelta[qq,1]/nrun;; endfor; "\\" "\\";
...
"beta   erho = ";; erho;; "&"  sumb/nrun/beta-1;;               "&"  sumbiv/nrun/beta-1;;               "&"  sumbab/nrun/beta-1;;                  for qq (1, nestim, 1); "&" sumg[1,qq]/nrun/beta-1;;          endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&"  median(bkeep[1:nrun,.])/beta-1;; "&"  median(bivkeep[1:nrun,.])/beta-1;; "&"  median(babkeep[1:nrun,.])/beta-1;;    for qq (1, nestim, 1); "&" median(gkeep[1:nrun,qq])/beta-1;; endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&"  sumerrb/nrun;;                   "&"  sumerrbiv/nrun;;                   "&"  sumerrbab/nrun;;                      for qq (1, nestim, 1); "&" sumerrg[1,qq]/nrun;;              endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&"  pcountb[1,1]/nrun;;              "&"  pcountbiv[1,1]/nrun;;              "&"  pcountbab[1,1]/nrun;;                 for qq (1, nestim, 1); "&" pcountg[1,qq]/nrun;;              endfor; "\\" "\\";; " [11pt] ";


for ee(1, nz, 1);
"alpha  erho = ";; erho;; "&" sumz[ee,1]/nrun/delta[ee,1]-1;;          "&" sumziv/nrun/delta[ee,1]-1;;                "&" sumzab/nrun/delta[ee,1]-1;;               for qq (1, nestim, 1); "&" sumd[ee,qq]/nrun/delta[ee,1]-1;;                       endfor; "\\" "\\";; " [2pt] ";
"median erho = ";; erho;; "&" median(zkeep[1:nrun,ee])/delta[ee,1]-1;; "&" median(zivkeep[1:nrun,.])/delta[ee,1]-1;;  "&" median(zabkeep[1:nrun,.])/delta[ee,1]-1;; for qq (1, nestim, 1); "&" median(dkeep[1:nrun,(ee-1)*nestim+qq])/delta[ee,1]-1;; endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&" sumerrz[ee,1]/nrun;;                     "&" sumerrziv/nrun;;                           "&" sumerrzab/nrun;;                          for qq (1, nestim, 1); "&" sumerrd[ee,qq]/nrun;;                                  endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&" pcountz[ee,1]/nrun;;                     "&" pcountziv[1,1]/nrun;;                      "&" pcountzab[1,1]/nrun;;                     for qq (1, nestim, 1); "&" pcountd[ee,qq]/nrun;;                                  endfor; "\\" "\\";; " [11pt] ";
endfor;

"rho2   erho = ";; erho;; "&" sumARR/nrun/rho2-1;;              "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" sump[1,qq]/nrun/rho2-1;;          endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&"  median(rkeep[1:nrun,.])/rho2-1;; "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" median(pkeep[1:nrun,qq])/rho2-1;; endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&" sumerrr/nrun;;                    "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" sumerrp[1,qq]/nrun;;              endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&" pcountr/nrun;;                    "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" pcountp[1,qq]/nrun;;              endfor; "\\" "\\";; " [11pt] ";

"tau2   erho = ";; erho;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" sumt[1,qq]/nrun/tau2-1;;          endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" median(tkeep[1:nrun,qq])/tau2-1;; endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" sumerrt[1,qq]/nrun;;              endfor; "\\" "\\";; " [2pt] ";
"       erho = ";; erho;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" pcountt[1,qq]/nrun;;              endfor; "\\" "\\";; " [11pt] ";

"------------------------------CMD----------------------------------------------";

"beta  erho = ";; erho;; "&"  sumb/nrun;;               "&"  sumbiv/nrun;;               "&"  sumbab/nrun;;                  for qq (1, nestim, 1); "&" sumg[1,qq]/nrun;;          endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&"  median(bkeep[1:nrun,.]);; "&"  median(bivkeep[1:nrun,.]);; "&"  median(babkeep[1:nrun,.]);;    for qq (1, nestim, 1); "&" median(gkeep[1:nrun,qq]);; endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&"  sumerrb/nrun;;            "&"  sumerrbiv/nrun;;            "&"  sumerrbab/nrun;;               for qq (1, nestim, 1); "&" sumerrg[1,qq]/nrun;;       endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&"  pcountb[1,1]/nrun;;       "&"  pcountbiv[1,1]/nrun;;       "&"  pcountbab[1,1]/nrun;;          for qq (1, nestim, 1); "&" pcountg[1,qq]/nrun;;       endfor; "\\" "\\";; " [11pt] ";


for ee(1, nz, 1);
"alpha erho = ";; erho;; "&" sumz[ee,1]/nrun;;          "&" sumziv/nrun;;                "&" sumzab/nrun;;               for qq (1, nestim, 1); "&" sumd[ee,qq]/nrun;;                       endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&" median(zkeep[1:nrun,ee]);; "&" median(zivkeep[1:nrun,.]);;  "&" median(zabkeep[1:nrun,.]);; for qq (1, nestim, 1); "&" median(dkeep[1:nrun,(ee-1)*nestim+qq]);; endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&" sumerrz[ee,1]/nrun;;       "&" sumerrziv/nrun;;             "&" sumerrzab/nrun;;            for qq (1, nestim, 1); "&" sumerrd[ee,qq]/nrun;;                    endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&" pcountz[ee,1]/nrun;;       "&" pcountziv[1,1]/nrun;;        "&" pcountzab[1,1]/nrun;;       for qq (1, nestim, 1); "&" pcountd[ee,qq]/nrun;;                    endfor; "\\" "\\";; " [11pt] ";
endfor;

"rho2  erho = ";; erho;; "&" sumARR/nrun;;                     "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" sump[1,qq]/nrun;;          endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&"  median(rkeep[1:nrun,.]);;        "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" median(pkeep[1:nrun,qq]);; endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&" sumerrr/nrun;;                    "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" sumerrp[1,qq]/nrun;;       endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&" pcountr/nrun;;                    "&" "  -----  ";;     "&" "  -----  ";;     for qq (1, nestim, 1); "&" pcountp[1,qq]/nrun;;       endfor; "\\" "\\";; " [11pt] ";

"tau2  erho = ";; erho;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" sumt[1,qq]/nrun;;          endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" median(tkeep[1:nrun,qq]);; endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" sumerrt[1,qq]/nrun;;       endfor; "\\" "\\";; " [2pt] ";
"      erho = ";; erho;; "&" "  -----  ";;      "&" "  -----  ";;     "&" "  -----  ";;         for qq (1, nestim, 1); "&" pcountt[1,qq]/nrun;;       endfor; "\\" "\\";; " [11pt] ";
...
"IV beta  erho = ";; erho;; "&"  sumbiv/nrun;;               "&"  sumbab/nrun;
"         erho = ";; erho;; "&"  median(bivkeep[1:nrun,.]);; "&"  median(babkeep[1:nrun,.]);
"         erho = ";; erho;; "&"  sumerrbiv/nrun;;            "&"  sumerrbab/nrun;
"         erho = ";; erho;; "&"  pcountbiv[1,1]/nrun;;       "&"  pcountbab[1,1]/nrun;

for ee(1, nz, 1);
"IV alpha erho = ";; erho;; "&" sumziv/nrun;;                "&" sumzab/nrun;
"         erho = ";; erho;; "&" median(zivkeep[1:nrun,.]);;  "&" median(zabkeep[1:nrun,.]);
"         erho = ";; erho;; "&" sumerrziv/nrun;;             "&" sumerrzab/nrun;
"         erho = ";; erho;; "&" pcountziv[1,1]/nrun;;        "&" pcountzab[1,1]/nrun;
endfor;
